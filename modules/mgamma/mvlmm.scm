...
(define-parameterized ((mph-initial %mph-initial) eval x y)
  ...
  (let* ((n-size (vec:length eval))
    ...
    (receive (dl ultveh ultvehi . rest)
        (eigen-proc vg ve)
      (receive (qi logdet-q)
          (calc-qi eval dl x)
            ...
            (dotimes (t (nr-iter)
              ...
              (let ((vg-save (mtx:copy! vg))
                ...
                (do ((scale 1.0 (/ scale 2))
                     ...
                     (do-not-exit-yet #t #f))
                    ((and (not do-not-exit-yet)
                          (or (not (or (not positive-definite?)
                                       (< logl-new logl-old)
                           ...
                           (do ((i 0 (1+ i))
                             ...
                             (mtx:with-column
                              ...
                              (let* ((vg vg-null)
                                     (ve ve-null)
                                     (b b-null)
                                     (logl-h1 (mph-em #:reml eval x y vg ve b)))
