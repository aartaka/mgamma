...
(define (py-max-likelihood-solution heritability x kva yt)
  (let* ((n-inds (mtx:rows x))
         (s (vec:copy! kva))
         (_ (vec:scale! s heritability))
         (_ (vec:add-constant! s (- 1 heritability)))
         ;; 1.0 / s
         (_ (vec:for-each (lambda (idx val)
                            (vec:set! s idx (/ 1 val)))
                          s))
         (xt (blas:gem x s #:transpose-a blas:+trans+))
         (xx (blas:gem xt x))
         (xx-i (linalg:invert xx))
         (x-i*xt (blas:gemm xx-i xt))
         (beta (blas:gemm x-i*xt yt))
         (x*beta (blas:gemm x beta))
         (_ (mtx:subtract! yt x*beta))
         (yt-t*s (blas:gemm yt s #:transpose-a blas:+trans+))
         (Q (blas:dot yt-t*s yt))
         (sigma (* Q (/ 1 (- n-inds (mtx:columns x))))))
    (mtx:free yt-t*s x*beta x-i*xt xx xt s)
    (values beta sigma Q xx-i)))

(define (py-log-likelihood heritability x kva yt)
  (let ((q (mtx:columns x))
        (n-inds (mtx:rows x))
        ;; TODO: Cleanup
        (kva-copy (vec:copy! kva)))
    (vec:scale! kva-copy heritability)
    (vec:add-constant! kva-copy (- 1 heritability))
    ;; np.log over the array
    (vec:for-each
     (lambda (i v)
       (vec:set! kva-copy i (log v)))
     kva-copy)
    (receive (beta sigma Q xx-i)
        (py-max-likelihood-solution heritability x kva yt)
      (values (* (+ (* n-inds (log (* 2 +pi+)))
                    (vec:sum kva-copy)
                    n-inds
                    (* n-inds (log (/ 1 (* n-inds Q)))))
                 -0.5)
              beta
              sigma
              xx-i))))

(define (make-py-ll-brent x kva yt)
  (lambda (h)
    (if (negative? h)
        1e6
        (- (py-log-likelihood h x kva yt)))))

(define (py-get-max heritability-steps heritability-options x kva yt)
  ;; list->vector for random access speed
  (let ((heritability-steps (list->vector heritability-steps))
        (heritability-options (list->vector heritability-options))
        (first-success #f))
    (do ((i 1 (1+ i)))
        ((= i (- (vector-length heritability-options) 2))
         (or first-success
             (if (> (vector-ref heritability-options 0)
                    (vector-ref heritability-options (1+ i)))
                 (vector-ref heritability-steps 0)
                 (vector-ref heritability-steps (1+ i)))))
      (when (and (< (vector-ref heritability-options (1- i))
                    (vector-ref heritability-options i))
                 (> (vector-ref heritability-options i)
                    (vector-ref heritability-options (1+ i))))
        (let ((optimized (root:optimize
                          root:+brent-solver+ 100 0.001
                          #:function (make-py-ll-brent x kva yt)
                          #:upper (vector-ref heritability-steps (1- i))
                          #:lower (vector-ref heritability-steps (1+ i)))))
          (when (and optimized
                     (not (nan? optimized))
                     ;; Only retain the first result.
                     (not first-success))
            (set! first-success optimized)))))
    (or first-success
        (vector-ref
         heritability-steps
         (if (> (vector-ref heritability-options 0)
                (vector-ref heritability-options
                            (1- (vector-length heritability-options))))
             0
             (1- (vector-length heritability-steps)))))))

;; Params:
;; self.K = kinship matrix
;; self.Kva = eigenvalues
;; self.Kve = eigenvectors
;; self.N = #inds
;; self.Y = phenotypes as matrix
;; self.X0 = covariates
;; self.Yt = Kve.T * Y
;; self.X0t = Kve.T * X0
;; self.X0t_stack = ???
;; self.q = #covariates
(define (pylmm pheno kinship kva kve cvt)
  (let* ((yt (blas:gemm kve pheno #:transpose-a kve))
         (x0t (blas:gemm kve cvt #:transpose-a kve))
         (q (mtx:columns x0t))
         (h-approximations (iota 100 0 0.01))
         (lls (map (cut py-log-likelihood <> x0t kva yt)
                   h-approximations))
         (hmax (py-get-max h-approximations lls x0t kva yt)))
    (receive (l beta sigma xx-i)
        (py-log-likelihood hmax x0t kva yt)
      (list hmax beta sigma l))))

(define (vec-useful-inds vec)
  (do ((ind 0 (1+ ind))
       (inds '()
             (cons (not (nan? (vec:get vec ind)))
                   inds)))
      ((= ind (vec:length vec))
       (reverse! inds))))

(define (useful-vec vec useful-inds)
  (let* ((n-useful (count identity useful-inds))
         (new-vec (vec:alloc n-useful)))
    (do ((useful-inds useful-inds (cdr useful-inds))
         (vec-i 0 (1+ vec-i))
         (i 0 (if (car useful-inds)
                  (1+ i)
        ...
        ((null? useful-inds))
      (when (car useful-inds)
        (vec:set! new-vec i (vec:get vec vec-i))))
    new-vec))

(define (py-tstat n beta var sigma q)
  (let* ((ts (/ beta (sqrt (* var sigma))))
         ;; stats.t.sf ???
         ;; (ps (* 2 (??? (abs ts) (- n q))))
         )
    ts))

(define (py-association x heritability kva yt)
  (receive (l beta sigma beta-var)
      (py-log-likelihood heritability x kva yt)
    (let ((q (mtx:rows beta)))
      ;; FIXME: Beta should be a vector?
      (py-tstat (mtx:rows x)
                (mtx:get beta (1- q) 0)
                (mtx:get beta-var (1- q) (1- q))
                sigma
                q))))

(define (pygwas geno-mtx markers kinship-mtx pheno-mtx cvt-mtx)
  (let* ((useful-individuals (useful-individuals pheno-mtx cvt-mtx))
         (useful-kinship (useful-kinship-mtx kinship-mtx useful-individuals))
         (useful-geno (useful-geno-mtx geno-mtx useful-individuals))
         (useful-pheno (useful-pheno-mtx pheno-mtx useful-individuals))
         (cvt-mtx (or cvt-mtx
                      (mtx:alloc (mtx:rows kinship-mtx) 1 1)))
         ;; FIXME: using `useful-pheno-mtx' here to not duplicate
         ;; logic. Should be a different name, though?
         (useful-cvt (useful-pheno-mtx cvt-mtx useful-individuals))
         (n-covariates (mtx:columns cvt-mtx))
         (n-useful-inds (mtx:rows useful-kinship))
         (n-markers (mtx:rows useful-geno))
         (per-snp-params (make-hash-table)))
    (receive (kva kve)
        (eigendecomposition-zeroed useful-kinship)
      (do ((markers markers (cdr markers))
           (snp 0 (1+ snp)))
          ((null? markers))
        (let* ((snp-inds (mtx:row->vec! useful-geno snp))
               (snp-useful-inds (vec-useful-inds snp-inds))
               (lmm-per-snp (pylmm
                             (useful-pheno-mtx useful-pheno snp-useful-inds)
                             (useful-kinship-mtx useful-kinship snp-useful-inds)
                             kva
                             kve
                             (useful-pheno-mtx useful-cvt snp-useful-inds))))
          (hash-set! per-snp-params (car markers) lmm-per-snp)
          (py-association geno-mtx ))))
    per-snp-params))
